...
export class PopupCancelPlanComponent implements OnInit, OnDestroy {
    ...
    onCancel() {
        this.loading = true;
        this.paymentService.cancelRecurringPaymentWorkflow()
            .subscribe((response: HttpResponse<ReturnPayment>) => {
                this.loading = false;
                this.dialogRef.close('success');
                this.snackUtil.openSnackBar('billing.cancelpopup.cancelsuccess', 10000, 'fa-check-circle');
    ...
    private processError(response: HttpErrorResponse) {
        ...
        this.snackUtil.openSnackBar(error, 30000, 'fa-times');
    }
